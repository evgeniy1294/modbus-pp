...
namespace modbus::cmd {
  ...
  ReadDiscreteInputs::ReadDiscreteInputs(std::uint8_t unit_id, std::size_t addr, std::size_t count)
  {
    _unit_id = unit_id;
    _addr    = addr;
    _count   = count;
...
namespace modbus::result {
  ...
  ReadDiscreteInputs::ReadDiscreteInputs( std::uint8_t unit_id,
  ...
  std::uint8_t ReadDiscreteInputs::GetCode()
  {
    return modbus::cmd::ReadDiscreteInputs::kCode;
  }
  ...
  std::size_t ReadDiscreteInputs::Serialize( std::uint8_t *pdu, std::size_t sz )
  {
    ...
    if ( ( sz >= RequestPduSize ) && (byte_count <= allowed_size) ){
      ...
      *pdu++ = modbus::cmd::ReadDiscreteInputs::kCode;
      *pdu++ = byte_count;
